...
module.exports.salesReport = async (req, res, next) => {
  try {
    ...
    const dateProject =
      groupBy === 'day'   ? { y: { $year: '$createdAt' }, m: { $month: '$createdAt' }, d: { $dayOfMonth: '$createdAt' } } :
      groupBy === 'week'  ? { y: { $year: '$createdAt' }, w: { $isoWeek: '$createdAt' } } :
                            { y: { $year: '$createdAt' }, m: { $month: '$createdAt' } };

    const groupId =
      groupBy === 'day'  ? { year: '$y', month: '$m', day: '$d' } :
      groupBy === 'week' ? { year: '$y', week: '$w' } :
                           { year: '$y', month: '$m' };
